...
pub fn malloc(self: *anyopaque, size: usize) callconv(.c) ?*anyopaque {
...

    {
        ...
        stbi.alloc_size_by_ptr.put(@intFromPtr(mem.ptr), size) catch @panic("out of memory");
    }
...
pub fn realloc(self: *anyopaque, in_ptr: ?*anyopaque, size: usize) callconv(.c) ?*anyopaque {
    ...
    if (in_ptr) |ptr| {
        ...
        const new_alloc = allocator.realloc(alloc[0..alloc_size], size) catch @panic("Out of memory");
